...
def next_page_exist_check(driver):
    if check_exists_by_css(driver, "div#page"):
        try:
            current_page_num = int(driver.find_element_by_css_selector('div#page strong span.pc').text)
            ...
            new_page_num = int(driver.find_element_by_css_selector('div#page strong span.pc').text)
...
def crawl_onion_address(driver, keyword, result_file):
    ...
    driver.find_element_by_id("kw").send_keys("site:"+keyword)
    ...
    while page_remain == True:
        if check_exists_by_css(driver, 'div#content_left') == True:
            link_list = driver.find_elements_by_css_selector('div#content_left div.result.c-container')
            for link in link_list:
                page_title = link.find_element_by_css_selector('h3.t').text
                page_title = page_title.replace("\"", " ")
                page_link = link.find_element_by_css_selector('div.f13').text
...
if __name__ == "__main__":
    ...
    link_list_file = open('new_linkset/' + collect_date + 'baidu_link_list.tsv', 'a', encoding='utf-8')
