...
def next_page_exist_check(driver):
    if check_exists_by_css(driver, "div#Sp1.side.mod"):
        try:
            current_page_num = int(driver.find_element_by_css_selector('div#Sp1.side.mod p strong').text)
            ...
            new_page_num = int(driver.find_element_by_css_selector('div#Sp1.side.mod p strong').text)
...
def crawl_onion_address(driver, keyword, result_file):
    ...
    driver.find_element_by_id("yschsp").send_keys("site:"+keyword)
    driver.find_element_by_id("yschsp").send_keys(Keys.RETURN)
    sleep(7)
    refresh_if_error_page(driver)

    page_remain = True
    while page_remain == True:
        if check_exists_by_css(driver, 'div#WS2m') == True:
            link_list = driver.find_elements_by_css_selector('div#WS2m div.w')
            for link in link_list:
                page_title = link.find_element_by_css_selector('div.hd a').text
                page_title = page_title.replace("\"", " ")
                page_link = link.find_element_by_css_selector('div.bd span').text
...
if __name__ == "__main__":
    ...
    driver.find_element_by_id("srchtxt").send_keys("test")
    driver.find_element_by_id("srchtxt").send_keys(Keys.RETURN)
    refresh_if_error_page(driver)

    sleep(1)

    link_list_file = open('new_linkset/' + collect_date + 'yahoo_link_list.tsv', 'a', encoding='utf-8')
